...
jQuery(function ($) {
  ...
  $('a[href^="#"]').click(function (e) {
    if ($(this).closest('.toc_title').length > 0) {
      return false;
    }
    const speed = 500;
    ...
    $.when(
      ...
      const diff = target.offset().top - headerHeight - 50;
      if (diff === position) {
      } else {
...
jQuery(window).scroll(function() {
  const winScrollTop = jQuery(window).scrollTop();
  const winH = jQuery(window).height();
  const $form = jQuery('#form');
  let formTop = 0;
  if ($form.length > 0) {
    formTop = jQuery('#form').offset().top;

    if(winScrollTop > 300) {
      if ((winScrollTop + winH) > formTop) {
        jQuery('.l-floating').fadeOut(400);
      } else {
        jQuery('.l-floating').fadeIn(400);
      }
    } else {
      jQuery('.l-floating').fadeOut(400);
    }
